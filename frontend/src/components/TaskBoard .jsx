...
import axios from "axios";
import ClipLoader from "react-spinners/ClipLoader";
import AddTaskModal from "../components/AddTaskModal"; // adjust path as needed
...
const TaskDetailModal = ({ task, onClose }) => (
  <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
    <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
      ...
      <button onClick={onClose} className="mt-6 bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-700">Close</button>
    </div>
  </div>
);
...
const TrelloBoard = () => {
  ...
  if (loading)
    return (
      <div className="flex justify-center text-2xl py-12">
        <h1>
          Initial loading, please wait...
          <ClipLoader
            color={color}
            loading={loading}
            cssOverride={override}
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          <input
            ...
            className="border p-3 rounded-lg w-80 focus:ring-2 focus:ring-blue-500"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <select
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex gap-8 overflow-x-auto">
          {Object.values(filteredColumns).map((column) => (
            <div key={column.id} className="min-w-[320px] max-w-[400px]">
              <h2 className="font-semibold text-lg bg-blue-600 text-white p-4 rounded-t-lg">
                {column.title}
              </h2>
              <Droppable droppableId={column.id}>
                {(provided) => (
                  <div
                    {...provided.droppableProps}
                    ref={provided.innerRef}
                    className="bg-white shadow-lg rounded-b-lg min-h-[200px] p-3"
                  >
                    {column.tasks.map((task, index) => (
                      <Draggable
                        key={task._id}
                      ...
                      >
                        {(provided) => (
                          <div
                            ...
                            {...provided.draggableProps}
                            {...provided.dragHandleProps}
                            className="bg-white p-6 mb-6 rounded-lg shadow-sm border hover:bg-blue-50"
                          >
                            <h3 className="font-semibold">{task.title}</h3>
